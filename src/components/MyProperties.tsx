...
const MyProperties: React.FC<MyPropertiesProps> = ({ isOpen, onClose, onBack }) => {
  ...
  const loadUserProperties = () => {
    ...
    // Filter properties by current user (assuming we store user info with property)
    const filtered = allProperties.filter((property: Property) => 
      property.owner?.email === user?.email || 
      property.agent?.name === user?.name
    );
              ...
              {userProperties.map((property) => (
                <Card key={property.id || property._id} className="overflow-hidden">
                  ...
                  <CardContent className="p-4">
                    <h3 className="font-semibold text-lg mb-2 capitalize">{property.title}</h3>
                    <div className="flex items-center text-gray-600 mb-2">
                      <MapPin className="h-4 w-4 mr-1" />
                      <span className="text-sm">{property.area}, {property.location}</span>
                    </div>
                    <div className="text-xl font-bold text-teal-600 mb-3">{property.price}</div>
